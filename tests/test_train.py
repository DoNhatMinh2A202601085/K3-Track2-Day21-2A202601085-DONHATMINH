import os
import json
import numpy as np
import pandas as pd
from src.train import train


FEATURE_NAMES = [
    "fixed_acidity", "volatile_acidity", "citric_acid", "residual_sugar",
    "chlorides", "free_sulfur_dioxide", "total_sulfur_dioxide", "density",
    "pH", "sulphates", "alcohol", "wine_type",
]


def _make_temp_data(tmp_path):
    """
    Tao dataset nho voi cung schema Wine Quality de su dung trong test.
    """
    rng = np.random.default_rng(0)
    n = 200

    X = rng.random((n, len(FEATURE_NAMES)))
    y = rng.integers(0, 3, size=n)

    df = pd.DataFrame(X, columns=FEATURE_NAMES)
    df["target"] = y

    train_path = str(tmp_path / "train.csv")
    eval_path = str(tmp_path / "eval.csv")
    df.iloc[:160].to_csv(train_path, index=False)
    df.iloc[160:].to_csv(eval_path, index=False)

    return train_path, eval_path


def test_train_returns_float(tmp_path):
    """Kiem tra ham train() tra ve mot so thuc nam trong [0.0, 1.0]."""
    train_path, eval_path = _make_temp_data(tmp_path)

    acc = train(
        {"n_estimators": 10, "max_depth": 3},
        data_path=train_path,
        eval_path=eval_path,
    )

    assert isinstance(acc, float)
    assert 0.0 <= acc <= 1.0


def test_metrics_file_created(tmp_path):
    """Kiem tra file outputs/metrics.json duoc tao sau khi huan luyen."""
    train_path, eval_path = _make_temp_data(tmp_path)
    train(
        {"n_estimators": 10, "max_depth": 3},
        data_path=train_path,
        eval_path=eval_path,
    )

    assert os.path.exists("outputs/metrics.json")
    with open("outputs/metrics.json") as f:
        metrics = json.load(f)
    assert "accuracy" in metrics
    assert "f1_score" in metrics


def test_model_file_created(tmp_path):
    """Kiem tra file models/model.pkl duoc tao sau khi huan luyen."""
    train_path, eval_path = _make_temp_data(tmp_path)
    train(
        {"n_estimators": 10, "max_depth": 3},
        data_path=train_path,
        eval_path=eval_path,
    )

    assert os.path.exists("models/model.pkl")


def test_bonus_multi_algorithms(tmp_path):
    """Bonus 2: Kiem tra ho tro nhieu thuat toan (extra_trees, gradient_boosting)."""
    train_path, eval_path = _make_temp_data(tmp_path)
    
    # Test ExtraTrees
    acc_et = train(
        {"model_type": "extra_trees", "n_estimators": 10, "max_depth": 3},
        data_path=train_path,
        eval_path=eval_path,
    )
    assert isinstance(acc_et, float) and 0.0 <= acc_et <= 1.0

    # Test GradientBoosting
    acc_gb = train(
        {"model_type": "gradient_boosting", "n_estimators": 10, "max_depth": 3},
        data_path=train_path,
        eval_path=eval_path,
    )
    assert isinstance(acc_gb, float) and 0.0 <= acc_gb <= 1.0


def test_bonus_performance_report_and_drift(tmp_path):
    """Bonus 3 & 5: Kiem tra tao file report.txt va thong tin phan phoi nhan."""
    train_path, eval_path = _make_temp_data(tmp_path)
    train(
        {"model_type": "random_forest", "n_estimators": 10, "max_depth": 3},
        data_path=train_path,
        eval_path=eval_path,
    )

    # Bonus 3: Kiem tra file outputs/report.txt
    assert os.path.exists("outputs/report.txt")
    with open("outputs/report.txt", encoding="utf-8") as f:
        content = f.read()
    assert "CONFUSION MATRIX" in content
    assert "CLASSIFICATION REPORT" in content

    # Bonus 5: Kiem tra class_distribution trong metrics.json
    with open("outputs/metrics.json") as f:
        metrics = json.load(f)
    assert "class_distribution" in metrics
